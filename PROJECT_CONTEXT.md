...
│   ├── event_emitter.py        # Structured event emitter — fire-and-forget HTTP POST to dashboard
...
│       │       └── bots/ (BotCard, BotModal, BotSelector, BotPeekDrawer)
│       ├── tailwind.config.js
│       └── vite.config.ts
```
...
- **Structured agent event streaming**: `utils/event_emitter.py` emits 7 event types (cycle_start, agent_result, decision, trade_execution, sl_tp_placed, cycle_cost) after each `graph.invoke`. Dashboard backend receives via `POST /api/internal/bot-event/{bot_id}` and relays to WebSocket clients on `/ws/bots/{bot_id}/events`. Bots page has eye-icon peek button per bot card that opens a 450px `BotPeekDrawer` slide-out with live event cards.
...
- **2026-04-02 (v15):** Paper/live separation audit + fixes. (1) `BotModal.tsx` mode toggle now syncs `exchange_testnet` alongside `trading_mode` — switching to Live sets testnet=0, switching to Paper sets testnet=1. Previously a "live" bot created via UI had `exchange_testnet=1` (testnet), tagging testnet trades as live. (2) `get_api_cost_stats()` / `/api/stats/api-costs` / `api.apiCosts()` / `Overview.tsx` — added `mode` filter throughout so API cost stats are correctly separated by paper/live. (3) Breakdown "bot" dimension `net_pnl` now uses mode-filtered API costs. (4) `create_bot` default exchange changed from `"deribit"` to `"dydx"`.
- **2026-04-02 (v14):** Guardian multi-exchange audit + fixes. `position_guardian.py` fully rewritten: now groups running bots by exchange, gets a separate adapter per exchange, and checks positions on each independently. `_orphan_tracker` keys changed from bare `ex_symbol` to `exchange:ex_symbol` to prevent cross-exchange key collisions. Missing-SL check now guarded by `not adapter.supports_native_sl_tp()` — skipped on Hyperliquid since exchange holds SL natively. Force-close on Hyperliquid now checks `fetch_open_orders` first; if SL/TP orders exist, defers to exchange. Emergency endpoint (`/api/emergency/close-all-positions`) updated to iterate all distinct exchanges from bots table. Start/end log lines added to every guardian cycle.
- **2026-04-02 (v13):** Fixed LangSmith project naming. `process_manager.py` was setting `LANGCHAIN_PROJECT` to `quantagent-{mode}-{symbol}`, creating a new project per bot. Now uses two fixed projects: `quantagent-paper` (trading_mode=="paper" or exchange_testnet truthy) and `quantagent-live` (live mainnet). All bots of the same environment share one LangSmith project.


- **2026-04-02 (v10):** Structured agent event streaming + Bots page quick-peek drawer. Created `utils/event_emitter.py` (fire-and-forget HTTP POST to dashboard, runs in daemon thread; 7 event types: cycle_start, agent_result, decision, trade_execution, sl_tp_placed, cycle_cost). `graph.py` emits all events after `graph.invoke` returns. Backend: `POST /api/internal/bot-event/{bot_id}` receives and relays events; `WebSocket /ws/bots/{bot_id}/events` streams them to clients; `bot_event_connections` dict manages per-bot connections. Frontend: new `BotPeekDrawer.tsx` slide-out 450px drawer from right with live event cards (agent signal badges, decision card with LONG/SHORT + entry/SL/TP, execution status); `BotCard` gains eye icon peek button (cyan hover); `Bots.tsx` manages `peekBotId` state.
...
| 2026-04-02 | Emit events after graph.invoke, not inside agent nodes | Keeps agent code clean and unmodified. Events are informational — they don't need per-agent granularity (timing precision), just cycle-level snapshots. Wrapping in try/except means emission failure can never affect trading. |
| 2026-04-02 | Separate /ws/bots/{id}/events from /ws/bots/{id}/logs | Raw logs are for debugging (terminal-style); structured events are for the peek drawer (pretty cards). Two different audiences and rendering modes. Both coexist — logs stay unchanged. |
