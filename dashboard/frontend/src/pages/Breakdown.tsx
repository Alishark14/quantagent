...
type Dimension = 'asset' | 'timeframe' | 'direction'

export default function Breakdown({ refreshTick }: Props) {
  const [dim, setDim] = useState<Dimension>('asset')
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([api.breakdown(dim, botId), api.exits(botId)])
      .then(([bd, ex]) => {
        setData(bd.data)
        setExits(ex)
        setError(null)
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted text-xs mt-0.5">Slice performance by asset, timeframe, or trade direction</p>
        </div>
